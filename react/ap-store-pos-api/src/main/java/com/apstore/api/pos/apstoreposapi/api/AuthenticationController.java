...
import com.apstore.api.pos.apstoreposapi.config.JwtUtils;
...
import com.apstore.api.pos.apstoreposapi.service.UserDetailsImpl;
import com.apstore.api.pos.apstoreposapi.vo.JwtResponse;
import com.apstore.api.pos.apstoreposapi.vo.LoginRequest;
import com.apstore.api.pos.apstoreposapi.vo.MessageResponse;
import com.apstore.api.pos.apstoreposapi.vo.SignupRequest;
...
public class AuthenticationController {
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
        ...
        if (strRoles == null) {
            ...
            strRoles.forEach(role -> {
                switch (role) {
                    case "admin":
                        Role adminRole = roleRepository.findByName(EnmRole.ROLE_ADMIN)
                                .orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                        roles.add(adminRole);

                        break;
                    case "mod":
                        Role modRole = roleRepository.findByName(EnmRole.ROLE_MODERATOR)
                                .orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                        roles.add(modRole);
    ...
    public ResponseEntity<?> authenticateUser(@Valid @RequestBody LoginRequest loginRequest) {
      ...
      System.out.println("=============Sign in" + loginRequest.toString());
